...
async function fetchWithReferer(url, timeoutMs = 15000, retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (isSocketError && attempt < retries) {
                logger_1.logger.warn(`fetchWithReferer socket error, retrying (${attempt + 1}/${retries}): ${url}`, err.message);
