...
const toolExecutor_1 = require("../chat/toolExecutor");
...
function parseShortcutCommand(input) {
    ...
    if (argsStr) {
        ...
        if (codeMatch) {
            code = codeMatch[1];
            extraArgs = argsStr.replace(codeMatch[0], '').trim();
        }
        else {
            extraArgs = argsStr;
        }
...
function formatToolResult(toolName, resultJson) {
    let result;
    try {
    ...
    switch (toolName) {
        ...
        case 'get_finance_summary': {
            ...
            for (const r of data.slice(0, 5)) {
                lines.push(`| ${r.reportDate} | ${r.eps} | ${r.roe} | ${r.revenueYoy} | ${r.netProfitYoy} | ${r.grossMargin} | ${r.netMargin} | ${r.debtRatio} |`);
            }
        ...
        case 'get_kline_summary': {
            const s = data;
            const tm = { increasing: '放量', decreasing: '缩量', stable: '平稳' };
            const cs = s.summary.periodChangePercent >= 0 ? '+' : '';
        ...
        case 'get_market_overview': {
            ...
            for (const q of data) {
                lines.push(`| ${q.name} | ${q.price} | ${q.changePercent} | ${q.changeAmount} | ${q.market} |`);
            }
            formatted = lines.join('\n');
            break;
        }
        ...
        case 'get_hot_stocks': {
            ...
            for (const s of data) {
                lines.push(`| ${s.code} | ${s.name} | ${s.price} | ${s.changePercent} | ${s.turnoverRate} |`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'get_sector_list': {
            ...
            for (const s of data) {
                lines.push(`| ${s.name} | ${s.price} | ${s.changePercent} | ${s.changeAmount} |`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'get_stock_news': {
            ...
            const lines = ['**个股新闻**', ''];
            for (const n of data) {
                lines.push(`- **${n.title}** (${n.source} ${n.time})`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'get_7x24_news': {
            ...
            for (const n of data) {
                ...
                lines.push(`- ${tag} ${n.content} _${n.createTime}_`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'get_research_reports': {
            ...
            const lines = ['**券商研报**', ''];
            for (const r of data) {
                lines.push(`- **${r.title}**`, `  ${r.orgName} | ${r.author} | ${r.publishDate} | 评级: ${r.rating}`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'search_stock': {
            ...
            for (const s of data) {
                lines.push(`| ${s.code} | ${s.name} | ${s.market} | ${s.type} |`);
            }
            formatted = lines.join('\n');
            break;
        }
        case 'open_chart': {
            formatted = data.message || '已打开图表';
            break;
        }
...
function getHelpText() {
    ...
    for (const cmd of SHORTCUT_COMMANDS) {
        if (cmd.name === '帮助' || seen.has(cmd.name)) {
            continue;
        }
        seen.add(cmd.name);
...
const mdRenderer = new marked_1.Marked();
mdRenderer.use({
    gfm: true,
    breaks: false,
    renderer: {
...
class AiChatPanel {
    ...
    renderMarkdown(content) {
        try {
            const html = mdRenderer.parse(content);
    ...
    async openConfigPanel() {
        ...
        const baseUrl = config.get('llmBaseUrl', '');
        const model = config.get('llmModel', 'gpt-3.5-turbo');
        this.panel?.webview.postMessage({
            type: 'showConfig',
  ...
  <script nonce="${nonce}">
    ...
    setupMessageHandler() {
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            // 获取所有工具定义
            const tools = (0, toolExecutor_1.getToolDefinitions)();
            try {
    ...
    async executeShortcutCommand(shortcut) {
        ...
        // 执行工具
        const resultJson = await (0, toolExecutor_1.executeTool)(command.toolName, args);
        // 通知前端工具执行完成
        this.panel.webview.postMessage({
            type: 'toolStatus',
            status: 'done',
     ...
     */
    async streamWithToolLoop(_tools) {
        ...
        while (rounds < MAX_TOOL_ROUNDS) {
            ...
            const llmMsgs = this.toLlmMessages().map(m => {
                ...
                const simplified = { role: m.role, content: m.content };
                return simplified;
            });
            ...
            for (const tc of toolCalls) {
                try {
                    ...
                    const result = await (0, toolExecutor_1.executeTool)(tc.name, args);
        ...
        if (this.watchlistProvider) {
            ...
            if (stocks.length > 0) {
                ...
                stockList = stocks.map(s => {
                    const safeName = (0, toolExecutor_1.sanitizeForPrompt)(s.name, 50);
                    const safeCode = (0, toolExecutor_1.sanitizeForPrompt)(s.code, 10);
