...
class LlmClient {
    ...
    static validateEndpoint(endpoint) {
        ...
        if (!isLocalhost && url.protocol !== 'https:') {
            throw new Error(`安全错误: 远程 API 地址必须使用 HTTPS 协议，以保护 API Key 不被明文传输。\n` +
                `当前地址: ${endpoint}\n` +
        ...
        for (let attempt = 0; attempt <= LlmClient.MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (error instanceof TypeError && error.message === 'fetch failed') {
                    const detailedError = new Error(`网络连接失败，请检查：\n` +
                ...
                if (attempt < LlmClient.MAX_RETRIES && this.isRetryableError(lastError)) {
                    console.warn(`[LLM] 流式请求失败（第 ${attempt + 1} 次），将自动重试:`, lastError instanceof Error ? lastError.message : lastError);
                    continue;
                }
                // 不可重试或已达最大重试次数，抛出错误
                throw lastError;
    ...
    async chat(messages, options) {
        ...
        for (let attempt = 0; attempt <= LlmClient.MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                // 判断是否可重试
                if (attempt < LlmClient.MAX_RETRIES && this.isRetryableError(lastError)) {
                    console.warn(`[LLM] 请求失败（第 ${attempt + 1} 次），将自动重试:`, lastError instanceof Error ? lastError.message : lastError);
                    continue;
                }
                // 不可重试或已达最大重试次数，抛出错误
                throw lastError;
...
/** 请求超时时间（毫秒） */
LlmClient.REQUEST_TIMEOUT_MS = 30000;
/** 最大重试次数 */
LlmClient.MAX_RETRIES = 1;
/** 初始重试延迟（毫秒），采用指数退避 */
LlmClient.INITIAL_RETRY_DELAY_MS = 1000;
/** 本地地址白名单（允许使用 HTTP 协议访问） */
LlmClient.LOCAL_HOSTNAMES = new Set(['localhost', '127.0.0.1', '::1']);
