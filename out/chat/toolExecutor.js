...
exports.KLINE_TOOLS = exports.STOCK_QUOTE_TOOLS = void 0;
exports.sanitizeForPrompt = sanitizeForPrompt;
exports.sanitizeToolOutput = sanitizeToolOutput;
exports.sanitizeToolData = sanitizeToolData;
exports.toolGetFinanceSummary = toolGetFinanceSummary;
exports.toolGetStockQuote = toolGetStockQuote;
exports.toolGetBatchQuotes = toolGetBatchQuotes;
exports.executeQuoteTool = executeQuoteTool;
exports.toolGetKlineSummary = toolGetKlineSummary;
exports.toolGetIntradayData = toolGetIntradayData;
exports.executeKlineTool = executeKlineTool;
 ...
 * @returns 消毒后的安全文本
 */
function sanitizeToolOutput(text, maxLength = 500) {
    if (!text || typeof text !== 'string') {
        return '';
    }
    // 截断
 ...
 * @returns 消毒后的数据副本
 */
function sanitizeToolData(data, textFields) {
    const sanitized = { ...data };
    for (const field of textFields) {
        if (typeof sanitized[field] === 'string') {
            sanitized[field] = sanitizeToolOutput(sanitized[field]);
        }
    }
    return sanitized;
}
 ...
 */
async function toolGetFinanceSummary(code) {
    // 参数校验：代码为空
    ...
    try {
        const indicators = await (0, eastmoney_1.getFinanceData)(trimmed);
        // 数据缺失：接口返回空数组
 ...
 */
function toStockQuoteResult(quote) {
    return {
        name: quote.name,
        code: quote.code,
 ...
 */
function toBatchQuoteItem(quote) {
    return {
        name: quote.name,
        code: quote.code,
        price: roundPrice(quote.price),
 ...
 */
async function toolGetStockQuote(code) {
    // 参数校验：空值
    if (!code || typeof code !== 'string') {
    ...
    try {
        const quote = await (0, sina_1.getRealtimeQuote)(trimmed);
 ...
 */
async function toolGetBatchQuotes(codes) {
    // 参数校验：非数组
    ...
    try {
        const quotes = await (0, sina_1.getBatchQuotes)(validCodes);
        // 过滤掉名称为空的无效数据
        const result = quotes
 ...
 * @returns ToolResult
 */
async function executeQuoteTool(toolName, args) {
    switch (toolName) {
 ...
 */
function getAmbiguousHint(code) {
 ...
 * - 数据不足 4 条时返回 stable
 */
function calcVolumeTrend(points) {
 ...
 */
async function toolGetKlineSummary(code, days = 30) {
    // 参数校验
    if (!code || typeof code !== 'string') {
    ...
    try {
        ...
        const series = await (0, eastmoney_1.getFullKlineData)(trimmed, isAmbiguous);
        // 截取最近 N 天的数据
 ...
 */
async function toolGetIntradayData(code) {
    // 参数校验
    if (!code || typeof code !== 'string') {
        return { success: false, error: '缺少股票代码参数' };
    }
    ...
    try {
        const series = await (0, sina_1.getIntradayData)(trimmed);
 ...
 * @returns ToolResult
 */
async function executeKlineTool(toolName, args) {
    switch (toolName) {
 ...
 */
async function executeTool(name, args, cmdExecutor) {
    try {
        let result;
        switch (name) {
            // --- 行情工具 ---
 ...
 * @returns            工具执行结果数组
 */
async function executeToolCalls(toolCalls, cmdExecutor) {
 ...
 */
async function executeFinanceSummary(code) {
 ...
 * open_chart — 打开图表
 */
async function executeOpenChart(code, chartType, cmdExecutor) {
    if (!code || !isValidStockCode(code)) {
        return { success: false, error: `无效的股票代码: "${code}"` };
    }
    const trimmed = code.trim();
    try {
        const executor = cmdExecutor || ((cmd, ...a) => vscode.commands.executeCommand(cmd, ...a));
 ...
 * search_stock — 搜索股票
 */
async function executeSearchStock(keyword) {
    if (!keyword || keyword.trim().length === 0) {
        return { success: false, error: '搜索关键词不能为空' };
    }
    try {
        const results = await (0, eastmoney_1.searchStocks)(keyword.trim());
        if (results.length === 0) {
 ...
 */
async function executeMarketOverview() {
    try {
        const quotes = await (0, market_1.getIndexQuotes)();
        return {
            success: true,
            data: quotes.map(q => ({
                name: q.name,
 ...
 */
async function executeMarketDistribution() {
    try {
        const data = await (0, market_1.getMarketDistribution)();
        return {
            success: true,
            data: {
 ...
 * get_hot_stocks — 热门股票
 */
async function executeHotStocks(rankType, count) {
    try {
        const validType = (['topGainers', 'topLosers', 'topTurnover'].includes(rankType)
            ? rankType
            : 'topGainers');
        const data = await (0, eastmoney_1.getHotStocks)(count, validType);
        return {
            success: true,
            data: data.map(s => ({
                code: s.code,
 ...
 */
async function executeSectorList() {
    try {
        const sectors = await (0, market_1.getIndustrySectors)(1);
        return {
            success: true,
 ...
 * get_stock_news — 个股新闻
 */
async function executeStockNews(code) {
    if (!code || !isValidStockCode(code)) {
        return { success: false, error: `无效的股票代码: "${code}"` };
    }
    try {
        const news = await (0, eastmoney_1.getStockNews)(code.trim(), 1, 5);
        return {
            success: true,
            data: news.map(n => ({
 ...
 * get_7x24_news — 快讯
 */
async function execute7x24News() {
    try {
        const news = await (0, sina_1.get7x24News)(1, 10);
        return {
            success: true,
            data: news.map(n => ({
 ...
 */
async function executeResearchReports(code) {
    if (!code || !isValidStockCode(code)) {
        return { success: false, error: `无效的股票代码: "${code}"` };
    }
    try {
        const reports = await (0, eastmoney_1.getResearchReports)(code.trim(), 1, 5);
        return {
            success: true,
            data: reports.map(r => ({
